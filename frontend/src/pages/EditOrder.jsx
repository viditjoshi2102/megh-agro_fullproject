...
export default function EditOrder() {
  ...
  useEffect(() => {
    ...
    setDealer({
      id:          orderData.dealer_id,
      dealer_name: orderData.dealer_name,
      dealer_code: orderData.dealer_code,
      city:        orderData.dealer_city,
      state:       orderData.dealer_state,
      gstin:       orderData.dealer_gstin,
    });
    setItems(
      (orderData.items || []).map(it => ({
        ...
        dealer_price: parseFloat(it.dealer_price),
        final_rate:   parseFloat(it.final_rate),
        quantity:     it.quantity,
        amount:       parseFloat(it.amount),
      }))
    );
    setTransporter({ name: orderData.transporter_name || '', contact: orderData.transporter_contact || '' });
    setNotes(orderData.notes || '');
    setOrderDate(orderData.order_date?.split('T')[0] || new Date().toISOString().split('T')[0]);
    setReady(true);
  }, [orderData, ready, id, navigate]);
  ...
  const mutation = useMutation({
    mutationFn: (payload) => orderApi.update(id, payload),
    onSuccess: () => {
  ...
  function handleProductSelect(idx, prod) {
    setItems(prev => prev.map((item, i) => i === idx
      ? { ...item, product_id: prod.id, product_code: prod.product_code, product_name: prod.product_name,
          hsn_code: prod.hsn_code, gst_percent: parseFloat(prod.gst_percent),
          dealer_price: parseFloat(prod.dealer_price), final_rate: parseFloat(prod.dealer_price),
          amount: parseFloat(prod.dealer_price) * item.quantity }
      : item
    ));
    setProductSearch('');
  }
  ...
  if (isLoading || !ready) return <div className="flex justify-center py-20"><Spinner size="lg" /></div>;

  return (
    <div className="max-w-3xl mx-auto">
      <PageHeader title={`Edit Order ${orderData?.order_no}`} subtitle="Order must be in pending state to edit"
        action={<Link to={`/orders/${id}`} className="btn-secondary py-1.5 px-3 text-sm">← Back</Link>}
      />
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
              ...
              {dealers && dealerSearch && (
                <div className="absolute top-full left-0 right-0 z-20 bg-white border border-gray-200 rounded-xl shadow-lg mt-1 max-h-60 overflow-y-auto">
                  {dealers.length === 0
                    ...
                    : dealers.map(d => (
                      <button type="button" key={d.id} onClick={() => { setDealer(d); setDealerSearch(''); }}
        ...
        <div className="card bg-brand-50 border-brand-200">
          ...
          <div className="space-y-1.5 text-sm">
            ...
            <div className="flex justify-between pt-2 border-t border-brand-200 text-base font-bold">
              <span>Grand Total</span><span className="text-accent-600">₹{formatCurrency(totals.grandTotal)}</span>
            </div>
          </div>
        </div>

        <div className="flex gap-3 pb-6">
          <Link to={`/orders/${id}`} className="btn-secondary flex-1 text-center">Cancel</Link>
